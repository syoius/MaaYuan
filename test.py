...
def main():
    ...
    task_detail = tasker.post_task("乘风破浪-启动", pipeline_override).wait().get()
...
class QuestionMatcher(CustomAction):
    ...
    def get_question(self, context: Context) -> str:
        ...
        result = context.run_recognition("乘风破浪-识别题目", img)
    ...
    def get_answer(self, context: Context) -> list[dict[str, list]]:
        ...
        for i in range(1, 5):  # 自动循环识别四个答案
            result = context.run_recognition(f"乘风破浪-识别选项_{i}", img)
